...
def test_accepts_insurance_certificate_doc_type(monkeypatch):
    """保险凭证是一等文档类型，不应被未知类型兜底成“其他”。"""

    def fake_call_llm_document(document_text, model=None):
        return LlmResult(
            parsed={
                "doc_type": "保险凭证",
                "title": "张三境外旅行保险凭证",
                "summary": "张三 2026-07-03 至 2026-07-18 境外旅行保险凭证",
                "parties": ["华泰财产保险股份有限公司", "张三"],
                "primary_date": "2026-07-03",
                "primary_amount": None,
                "fields": [{"label": "承保公司", "value": "华泰财产保险股份有限公司"}],
                "amounts": [],
                "key_dates": [{"label": "保险期间起始日", "date": "2026-07-03"}],
                "person_identities": [],
                "seals": [],
                "obligations": [],
                "sub_agreements": [],
    ...
    monkeypatch.setattr(
        "contract_archive.extraction.document_extractor.call_llm_document",
        fake_call_llm_document,
    )

    env = extract_document("华泰财产保险股份有限公司 张三 境外旅行保险", llm_enabled=True)

    assert env.doc_type == "保险凭证"
    assert env.completeness is None


def test_accepts_travel_material_doc_type(monkeypatch):
    """行程/登船须知是一等文档类型，不应被未知类型兜底成“其他”。"""

    def fake_call_llm_document(document_text, model=None):
        return LlmResult(
            parsed={
                "doc_type": "旅行资料",
                "title": "Red Sea Aggressor IV 船宿登船与行程须知",
                "summary": "Red Sea Aggressor IV 船宿行程资料，含登船时间、接驳、签证与费用。",
                "parties": ["Aggressor Adventures"],
                "primary_date": None,
                "primary_amount": None,
                "fields": [
                    {"label": "船名", "value": "Red Sea Aggressor IV"},
                    {"label": "母港", "value": "Port Ghalib Marina"},
                ],
                "amounts": [{"label": "港口费", "text": "$45", "unit": "USD/人"}],
                "key_dates": [{"label": "更新日期", "date": "2026-03-31"}],
                "person_identities": [],
                "seals": [],
                "obligations": [],
                "sub_agreements": [],
    ...
    env = extract_document("Red Sea Aggressor IV Know Before You Go Port Ghalib", llm_enabled=True)

    assert env.doc_type == "旅行资料"
    assert env.completeness is None
