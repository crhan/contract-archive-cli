...
3. 表格里按行列对齐读，别错位。
4. **投保人/被保险人 取投保单的结构化栏位**（"投保人姓名"/"被保险人姓名"/投保人资料/被保险人资料栏）。**签名栏可能是代签或监护人代签**（投保单常由家属代签），**不要仅凭签名栏推翻结构化字段**；结构化栏与签名不一致时，保留结构化栏的姓名并把 low_confidence 设为 true。
5. 只输出 JSON，不要解释、不要 Markdown 代码块标记：
...
def _normalize_value(v: str) -> str:
    """轻量归一化用于一致性比较：去空白/大小写；**仅对含数字的值**剥币种/单位、万元→万、整年月→年。
    ...
    整年月→年（"12个月"→"1年"）：保险期间常一处写"1年"、另一处写"12个月"，二者等价。归一后
    _agree 直接判一致、省一次评判；scorer 也不再把同值不同写法误判失配（仅整 12 倍折，"13个月"不动）。
    """
    ...
    if any(ch.isdigit() for ch in s):
        for junk in ("元整", "圆整", "元", "圆", "￥", "¥", "rmb", "人民币", "整"):
            s = s.replace(junk, "")
        s = s.replace("万元", "万")
        m = re.fullmatch(r"(\d+)个?月", s)
        if m and int(m.group(1)) % 12 == 0:
            s = f"{int(m.group(1)) // 12}年"
    return s
